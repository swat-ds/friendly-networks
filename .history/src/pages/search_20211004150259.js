...
const search = ({location, data}) => {
    ...
    if (typeof window !== "undefined" && typeof document !== "undefined") {
      ...
      let jFuseResult = journalFuse.search(searchQuery);
      ...
      let cFuseResult = constellationFuse.search(searchQuery);
    ...
    return (
      <Layout>
        ...
        <h6 className="general-text">{parsedJournals.length}</h6>
        <h6 className="general-text">{journalResult.length}</h6>
        <h className="general-text">
          {constellationResult.length} results for "{query}"
        </h>

        <br />
        <h6>Journal Results</h6>
