...
const Network = ({ nodesInJSON, linksInJSON, centralFigure}) => {
  ...
  let noHuntLinks = [];
   ...
   useEffect(() => {
     ({noHuntNodes, noHuntLinks} = getNodesWithoutHunt(nodesInJSON, linksInJSON, centralFigure));
  ...
  const [remove, setClicker] = useState(false);
  ...
  }, [nodes, links, remove]); //End of useEffect()
  ...
  return (
    <Row id="main-row">
      <Col id="mainContainer">
        <Button variant={remove? "success" : "danger"} onClick={() => setClicker(!remove)}>
          <span className="general-text">{remove? "Add Hunt": "Remove Hunt"}</span>
        </Button>
        <svg
