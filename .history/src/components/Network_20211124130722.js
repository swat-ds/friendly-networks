...
function getNodesWithoutHunt(nodes, links, huntId){
  ...
  let filteredLinks = links.filter((link) => {
   ...
   console.log("nodes in json length:", nodes.length);
    console.log("links in json length:", links.length);
    console.log("no Hunt nodes length:", filteredNodes.length);
    console.log("no Hunt links :", filteredLinks.length);
...
const Network = ({ nodesInJSON, linksInJSON, centralFigure}) => {
  ...
  //  const [noHuntNodes, setNoHuntNodes] = useState([]);
  //  const [noHuntLinks, setNoHuntLinks] = useState([]);

  const {noHuntNodes, noHuntLinks} =  useMemo(() => {
    return getNodesWithoutHunt(
       nodesInJSON,
       linksInJSON,
       centralFigure
     );
   }, [linksInJSON, nodesInJSON, centralFigure]);
